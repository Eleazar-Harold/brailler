from .app_conv import BrCnvx
from .forms import T2BForm, B2TForm
    ...
    if request.session['text_to_braille']:
        ...
        if fs.exists(filename):
            with fs.open(filename) as pdf:
                ...
                response['Content-Disposition'] = 'attachment; filename={}'.format(doc_name)
                try:
...
def pdf_printer(doc_name=None, input=None):
    doc = SimpleDocTemplate("/tmp/{}".format(doc_name),
                            pagesize=letter,
                            rightMargin=72,
                            leftMargin=72,
                            topMargin=72,
